...
class SSDPServer:
    ...
    def ssdp_main_thread(self):
        ...
        if sys.platform == 'win32':
            ...
            try:
                self.sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEPORT, 1)
                logger.info("SSDP set SO_REUSEPORT")
            ...
            try:
                self.sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
                logger.info("SSDP set SO_REUSEADDR")
            except socket.error as e:
        ...
        self.ssdp_byebye()
        for ip, mask in self.ip_list:
            logger.info(f"drop membership {ip}")
            mreq = socket.inet_aton(SSDP_ADDR) + socket.inet_aton(ip)
            try:
    ...
    def datagram_received(self, data, host_port):
        ...
        if cmd[0] != 'NOTIFY':
            logger.info(f'SSDP command {cmd[0]} {cmd[1]} - from {host}:{port}')
        if cmd[0] == 'M-SEARCH' and cmd[1] == '*':
            # SSDP discovery
            logger.info(f'M-SEARCH * {data}')
    ...
    def discovery_request(self, headers, host_port):
        ...
        logger.info(f'Discovery request from ({host}:{port}) for {headers["st"]}')
        ...
        for i in devices:
            if i['NT'] == headers['st'] or headers['st'] == 'ssdp:all':
                ...
                logger.info(f'send discovery {i["USN"]} response  to {destination}')
                logger.info(resp)
